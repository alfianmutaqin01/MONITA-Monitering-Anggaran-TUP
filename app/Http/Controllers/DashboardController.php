...
class DashboardController extends Controller
{
    ...
    private function getLastDataRow($service, $sheetName, $column = 'C')
    {
        $range = "{$sheetName}!{$column}6:{$column}1000";
        $response = $service->spreadsheets_values->get($this->spreadsheetId, $range, [
            'valueRenderOption' => 'UNFORMATTED_VALUE'
        ]);
        
        $values = $response->getValues() ?? [];
        $lastRow = 5; // Default ke baris 5 (header)
    ...
    private function getTotalSaldo($service, $sheetName)
    {
        $lastDataRow = $this->getLastDataRow($service, $sheetName, 'C');
        
        $totalRow = $lastDataRow + 1;
        
        $range = "{$sheetName}!H{$totalRow}";
        
        return $this->getCellValue($service, $range);
    }
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Gunakan method dinamis untuk mendapatkan saldo TOTAL
            $saldoTW1 = $this->getTotalSaldo($service, 'SUMMARY TW I');
            $saldoTW2 = $this->getTotalSaldo($service, 'SUMMARY TW II');
            $saldoTW3 = $this->getTotalSaldo($service, 'SUMMARY TW III');
            $saldoTW4 = $this->getTotalSaldo($service, 'SUMMARY TW IV');
